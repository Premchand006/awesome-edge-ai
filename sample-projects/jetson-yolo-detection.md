...
4. **Export to TensorRT for speed**, then run the engine:
   ...
   ```
   - **Why faster:** TensorRT fuses layers and uses FP16/INT8 tuned for your exact GPU. See [TensorRT](../runtimes-and-frameworks/README.md).
...
3. Read detections from **`NvDsBatchMeta`** to turn boxes into events (counts, alerts). Pipeline concepts: [GStreamer & DeepStream](../pipelines/README.md).
...
- Move perception into a robot with **[Isaac ROS](../pipelines/README.md)**.
- Try **INT8 quantization** for more FPS ([optimization](../model-optimization/README.md)).
